...
class AnyPlayCommand(QiCellCommand, QiTriggerCommand, ABC):
    ...
    def __init__(self, cell, pulse: _QiPulse):
        super().__init__(cell)
        self.pulse = pulse
...
class PlayFluxCommand(AnyPlayCommand):
    ...
    def __init__(self, coupler: QiCoupler, pulse: _QiPulse) -> None:
...
class RotateFrameCommand(AnyPlayCommand):
    ...
    def __init__(self, cell, angle: float):
        ...
        pulse = _QiPulse(0, phase=-1 * angle)
...
class ForRangeCommand(QiCommand):
    def __init__(
        self,
        var: _QiVariableBase,
        start: QiExpression | int | float,
        end: QiExpression | int | float,
        step: QiExpression | int | float,
        body: list[QiCommand],
    ):
        super().__init__()
        ...
        if isinstance(start_expr, _QiVariableBase):
            self.add_associated_variable(start_expr)

            if start_expr.id == var.id:
                raise RuntimeError("Loop variable can not be used as start value")

        if isinstance(end_expr, _QiVariableBase):
            self.add_associated_variable(end_expr)

            if end_expr.id == var.id:
